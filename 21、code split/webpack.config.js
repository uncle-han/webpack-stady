...
/*
    开启tree shaking的方式  ： 去除无用的代码
    1、es6模块引用的方式  2、开启生成模式
    有点：减少代码体积
    注意点：
    package.json中的配置
        "sideEffects": false  所有的代码都没有副作用（都可以进行tree shaking）
        在某些webpack的版本中，会把css文件作为无用代码，不被打包
        设置:
        "sideEffects": ["*.css", "*.less"] 把css，less文件列入打包文件
*/
...
module.exports = {
    ...
    output: {
        ...
        filename: 'build.[contenthash:5].js'
    },
    module: {
        rules: [
            {
                ...
                oneOf: [
                    {
                        ...
                        loader: 'url-loader',
                        options: {
                            limit: 8 * 1024, // 对小于8kb的进行Bash64处理
    ...
    plugins: [
        ...
        new MiniCssExtractPlugin({
            filename: '/css/index.[contenthash:5].css'
        }),
